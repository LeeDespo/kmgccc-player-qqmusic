...
enum PlaybackStartPolicy: Equatable {
    ...
    /// An external driver owns the playing order and feeds the queue itself, so
    /// the engine must advance linearly and not second-guess it.
    ///
    /// Used by the online source: it knows the whole list, decides the order
    /// (shuffling it when the user asked for shuffle), and hands tracks over one
    /// at a time. Letting the engine shuffle as well would apply a second,
    /// independent shuffle over the same tracks and inject its own picks from
    /// whichever tracks happen to be queued — which played out as "shuffle only
    /// covers what has been downloaded".
    ///
    /// Unlike the forced modes this does not override what the UI shows: the
    /// user's chosen mode stays visible, because the ordering they asked for is
    /// still what they get — it is just applied by the caller.
    case externalOrder

    var isTemporaryOverride: Bool {
        switch self {
        case .useSavedMode, .externalOrder:
            return false
    ...
    func resolvedMode(settings: AppSettings = .shared) -> PlaybackOrderMode {
        ...
        case .forceSequentialTemporary, .externalOrder:
            return .sequence
        case .forceShuffleTemporary:
            return .shuffle
        }
...
public final class AppSettings {
    ...
    private enum QQMusicKeys {
        ...
        static let songCacheLimitEnabled = "qqMusicSongCacheLimitEnabled"
        static let songCacheLimitGB = "qqMusicSongCacheLimitGB"
        static let songCacheReclaimPercent = "qqMusicSongCacheReclaimPercent"
        static let otherCacheLimitEnabled = "qqMusicOtherCacheLimitEnabled"
        static let otherCacheLimitGB = "qqMusicOtherCacheLimitGB"
        static let otherCacheReclaimPercent = "qqMusicOtherCacheReclaimPercent"
    }
    ...
    /// Read a `Double` setting, falling back when unset. `double(forKey:)`
    /// returns 0 for a missing key, and 0 is not a meaningful limit, so
    /// presence is tested rather than the value.
    private func qqMusicDouble(_ key: String, default fallback: Double) -> Double {
        UserDefaults.standard.object(forKey: key) == nil
            ? fallback
    ...
    // MARK: QQ Music cache budgets
    //
    // Two separate budgets, because the two kinds of data have different value
    // and different cost to replace:
    //
    //   songs  — downloaded audio that the app fetched on its own to keep
    //            playback ahead. Reclaimable, since it can be fetched again.
    //   other  — catalogue JSON and cover images, all of it re-derivable.
    //
    // Audio the *user* asked for is deliberately not part of either budget: it
    // is their library, not a cache. See `QQMusicDownloadOrigin`.

    /// Whether automatic-downloaded audio is subject to a size limit.
    var qqMusicSongCacheLimitEnabled: Bool {
        get {
            access(keyPath: \.qqMusicSongCacheLimitEnabled)
            return qqMusicBool(QQMusicKeys.songCacheLimitEnabled, default: false)
        }
        set {
            withMutation(keyPath: \.qqMusicSongCacheLimitEnabled) {
                UserDefaults.standard.set(newValue, forKey: QQMusicKeys.songCacheLimitEnabled)
            }
        }
    }

    /// Upper bound for automatic-downloaded audio, in GB.
    var qqMusicSongCacheLimitGB: Double {
        get {
            access(keyPath: \.qqMusicSongCacheLimitGB)
            return qqMusicDouble(QQMusicKeys.songCacheLimitGB, default: 1)
        }
        set {
            withMutation(keyPath: \.qqMusicSongCacheLimitGB) {
                UserDefaults.standard.set(newValue, forKey: QQMusicKeys.songCacheLimitGB)
            }
        }
    }

    /// How much of the limit to keep once it is exceeded, as a percentage.
    ///
    /// Reclaiming to the limit itself would mean evicting again on the very next
    /// download; trimming to a lower water mark makes eviction occasional
    /// instead of continuous. 70% of 1 GB leaves 300 MB of headroom.
    var qqMusicSongCacheReclaimPercent: Int {
        get {
            access(keyPath: \.qqMusicSongCacheReclaimPercent)
            return qqMusicInt(QQMusicKeys.songCacheReclaimPercent, default: 70)
        }
        set {
            withMutation(keyPath: \.qqMusicSongCacheReclaimPercent) {
                UserDefaults.standard.set(newValue, forKey: QQMusicKeys.songCacheReclaimPercent)
            }
        }
    }

    /// Whether the catalogue/artwork cache is subject to a size limit.
    var qqMusicOtherCacheLimitEnabled: Bool {
        get {
            access(keyPath: \.qqMusicOtherCacheLimitEnabled)
            return qqMusicBool(QQMusicKeys.otherCacheLimitEnabled, default: false)
        }
        set {
            withMutation(keyPath: \.qqMusicOtherCacheLimitEnabled) {
                UserDefaults.standard.set(newValue, forKey: QQMusicKeys.otherCacheLimitEnabled)
            }
        }
    }

    /// Upper bound for the catalogue and artwork cache, in GB.
    var qqMusicOtherCacheLimitGB: Double {
        get {
            access(keyPath: \.qqMusicOtherCacheLimitGB)
            return qqMusicDouble(QQMusicKeys.otherCacheLimitGB, default: 1)
        }
        set {
            withMutation(keyPath: \.qqMusicOtherCacheLimitGB) {
                UserDefaults.standard.set(newValue, forKey: QQMusicKeys.otherCacheLimitGB)
            }
        }
    }

    /// How much of the other-cache limit to keep once exceeded, as a percentage.
    var qqMusicOtherCacheReclaimPercent: Int {
        get {
            access(keyPath: \.qqMusicOtherCacheReclaimPercent)
            return qqMusicInt(QQMusicKeys.otherCacheReclaimPercent, default: 70)
        }
        set {
            withMutation(keyPath: \.qqMusicOtherCacheReclaimPercent) {
                UserDefaults.standard.set(newValue, forKey: QQMusicKeys.otherCacheReclaimPercent)
            }
        }
    }
